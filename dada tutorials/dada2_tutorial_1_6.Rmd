...
> tries to figure out which sequences are replicates and which are truly different sequences, using the error model - if there are 10,000 of the same (wrong/error) sequence, don't need to compare each of them, will make the change for all of the same. 
...
```
> FilterAndTrim left out one file that failed filtering
> need to make a new list of sample names... (see below)
...
> Could also/instead do (but don't run):
...
> Filter -> Derep -> Infer
> what is the difference between unique (derep'd) sequences and inferred samples?
...
> now ready to merge the F and R reads
...
> 233 represents a 253 amplicon with 10 bp trim (on each F/R)
> there is natural variation in amplicon length
...
> where parts of sequences get mismatched (like head of a sequence + tail of another sequence or head + head of the same sequence)
...
> Josh does differently than dada2 tutorial 

> extracting numbers for each sample from each dataframe 
>> when did we make these dataframes? doing that here below 
...
> how could I subset the data to find any that are identified at genus and/or species level?
>> maybe: taxa.print[Genus, NA=FALSE] ... no this wouldn't be correct. 

> attempt:

```{r}
taxa.print.species = taxa.print[complete.cases(taxa.print), ]
head(taxa.print.species)
```
...
> need to fix some things to get ready for phyloseq..
> need to get rid of some Qiime stuff
> wants sample names as rownames
